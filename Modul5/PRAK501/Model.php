...
function readData($nama_tabel)
{
    ...
    if (!empty($result)) {
        ...
        elseif ($nama_tabel == "peminjaman") {
            foreach ($result as $hasil) {
                echo "<tr>";
                echo "<td class='text-center'>" . $hasil["id_peminjaman"] . "</td>";
...
// Mengambil Data
function getMember($dbh)
{
    $query = koneksi()->prepare("SELECT * from member");
    $query->execute();
    $hasil = $query->fetchAll(PDO::FETCH_ASSOC);
    return $hasil;
}

function getBuku($dbh)
{
